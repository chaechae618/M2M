...
type MessageKind = "text" | "mentor_suggestion" | "mentor_request_status" | "mentor_request_editor";
  ...
  },
  {
    ...
    content: (
      <>
        그럼 꽤 쓸 수 있어요.
        <br />
        핵심은 평균 조회수가 아니라 “왜 그 콘텐츠만 튀었는지 분석했는가”예요.
      </>
    ),
  },
];
...
export default function ChatPage() {
  ...
  const showEditor = messages.some((message) => message.kind === "mentor_request_editor");
  ...
  function handleMentorAccept() {
    appendMessages([
      {
        id: `u-mentor-${Date.now()}`,
        role: "user",
        kind: "text",
        compact: true,
        text: "엉 보낼래",
  ...
  function handleEditorComplete() {
    setMessages((current) => current.filter((message) => message.kind !== "mentor_request_editor"));
  }
  ...
  return (
    <main className="flex h-dvh min-h-[720px] w-full bg-[#f9f9f9] py-2 pr-2 text-[#242424]">
      ...
      <section className="relative flex min-w-0 flex-1 overflow-hidden rounded-2xl border border-[#eeeeee] bg-[#fefefe] shadow-[0_6px_20px_rgba(68,74,83,0.12)]">
        <div className="relative z-10 flex min-h-0 w-full flex-col items-center px-[clamp(24px,10vw,180px)]">
            ...
            <div className="flex min-h-0 w-full flex-1 flex-col gap-7 py-5">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto">
                <MessageList
                  ...
                  onMentorAccept={handleMentorAccept}
...
function mockAssistantReply(input: string): ChatMessage[] {
  if (input.includes("조회수") || input.includes("작")) {
    return [
      {
        id: `a-mentor-${Date.now()}`,
        role: "assistant",
        kind: "mentor_suggestion",
      },
    ];
  }
...
function ChatTitleBar({ onBack }: { onBack: () => void }) {
  return (
    <header className="flex h-8 w-full shrink-0 items-center gap-2">
      ...
      <h1 className="min-w-0 flex-1 text-center text-[18px] font-semibold leading-[1.55]">비전공자 PM 준비</h1>
...
function MessageList({
  ...
  return (
    <div className="flex w-full flex-col gap-2 pb-4 text-[16px] font-medium leading-[1.6]">
      {!showEditor ? (
        <button
          type="button"
          onClick={() => {
          ...
          className="mb-2 self-start rounded-lg bg-[#f7f7f7] px-3 py-2 text-[14px] text-[#585858]"
        >
          멘토 제안 응답 예시 보기
        </button>
      ) : null}
...
function MessageRenderer({
  ...
  if (message.kind === "mentor_suggestion") {
    return <MentorSuggestionMessage onAccept={onMentorAccept} />;
  }
...
function MentorSuggestionMessage({ onAccept }: { onAccept: () => void }) {
  return (
    <AssistantBlock>
      <p>
        그 걱정은 현실적이에요.
        <br />
        마케팅 포트폴리오에서는 숫자의 크기보다, 작은 데이터를 어떻게 해석했는지가 더 중요할 때가 많아요.
      </p>
      <p className="mt-5">
        이건 <strong className="font-bold">현업 마케터</strong>에게 물어보면 더 정확한 답을 받을 수 있어요.
        <br />
        특히 “성과가 작을 때 포트폴리오에서 지표를 어떻게 보여줘야 하는지”는 실무자 관점이 필요해요.
      </p>
      <button
        type="button"
        onClick={onAccept}
        className="mt-3 flex max-w-[600px] items-center gap-2 rounded-[28px] bg-[radial-gradient(circle_at_center,#fff3d6_0%,#fffbee_100%)] px-3 py-1.5 text-left"
      >
        <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-[#ffc456] text-[12px] font-bold text-white">!</span>
        <span className="bg-[linear-gradient(90deg,#fca302_4%,#ff8400_61%,#ff9e17_100%)] bg-clip-text text-[16px] text-transparent">
          현업 콘텐츠 마케팅 멘토에게 이 고민을 보내볼까요?
        </span>
      </button>
      <MessageActions />
    </AssistantBlock>
  ...
  return (
    <AssistantBlock>
      <button
        ...
        className="bg-[linear-gradient(90deg,#959595_0%,#cacaca_15%,#bbbbbb_33%)] bg-clip-text text-left text-[16px] text-transparent"
      >
        {text}
      </button>
      <MessageActions hidden />
...
function MentorRequestEditor({ onComplete }: { onComplete: () => void }) {
  return (
    <div className="flex h-full w-full flex-col gap-2 pb-4">
      <p className="text-[15px] font-normal leading-none text-[#9e9e9e]">질문 직접 수정하기</p>
...
function ChatComposer({
  ...
  return (
    <form
      className="w-full rounded-2xl border border-[#eeeeee] bg-white px-6 pb-4 pt-6 shadow-[0_6px_10px_rgba(68,74,83,0.12)]"
      onSubmit={(event) => {
    ...
    >
      <textarea
        ...
        rows={1}
        className="block min-h-[26px] w-full resize-none border-0 bg-transparent text-[16px] font-medium leading-[1.6] text-[#242424] outline-none placeholder:text-[#9e9e9e]"
        onKeyDown={(event) => {
